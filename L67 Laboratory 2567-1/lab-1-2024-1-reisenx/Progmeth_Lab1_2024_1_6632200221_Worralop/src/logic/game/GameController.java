...
public class GameController {
    ...
    public void endDay(){
        for (int i = 0; i < players.size(); i++) {
        	int ENERGY = players.get(i).getEnergy();
        	int HP = players.get(i).getStatus().getHp();
        	if(ENERGY < 3) {
        		try {
        			players.get(i).getStatus().setHp( Math.max(0, HP-Math.abs(ENERGY - 3)) );
        		}
        		catch(BadStatusException e) {}
        		players.get(i).setEnergy(0);
        	}
        	else { players.get(i).setEnergy(ENERGY - 3); }
        }
    }
