...
public class GameLogic {
	...
	public void startCountDownTimer(int pl) {
		// Create separate thread and run
		Thread timerThread = new Thread(() -> {
			try { runCountDownTimer(pl); }
	...
	public void runCountDownTimer(int pl) throws InterruptedException {
		...
		if(pl==0) {
			while (gameStart&&isOTurn && !plTimer.isTimerEmpty()) {
				Thread.sleep(20);
				// User Platform.runlater() to update timer UI
				Platform.runLater(() -> {
	...
	private void runWinningPattern()  {
		// Create separate thread and run
		Thread winningThread = new Thread(() -> {
			try { winningPattern(score); }
			catch(InterruptedException e) {
				throw new RuntimeException(e);
			}
		});
